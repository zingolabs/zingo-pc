...
export class TokenCatalog {
  ...
  async listRoutableTokens(direction: "outbound" | "inbound"): Promise<TokenEntryType[]> {
    ...
    const filterSet = direction === "outbound" ? this.routableOutbound : this.routableInbound;
  ...
  private async fetchAndProcess(): Promise<TokenEntryType[]> {
    ...
    const [tokensResult, swapToResult, swapFromResult] = await Promise.allSettled([
      this.client.tokens(),
    ...
    if (swapToResult.status === "fulfilled" && Array.isArray(swapToResult.value)) {
      // Lowercased so the match in `listRoutableTokens` is case-insensitive.
      this.routableOutbound = new Set(swapToResult.value.map((id) => id.toLowerCase()));
    } else if (swapToResult.status === "rejected") {
      const msg = swapToResult.reason instanceof Error ? swapToResult.reason.message : String(swapToResult.reason);
      console.log("TokenCatalog: /swapTo failed, falling back to full catalog:", msg);
    }

    if (swapFromResult.status === "fulfilled" && Array.isArray(swapFromResult.value)) {
      // Lowercased so the match in `listRoutableTokens` is case-insensitive.
      this.routableInbound = new Set(swapFromResult.value.map((id) => id.toLowerCase()));
    } else if (swapFromResult.status === "rejected") {
      const msg =
        swapFromResult.reason instanceof Error ? swapFromResult.reason.message : String(swapFromResult.reason);
      console.log("TokenCatalog: /swapFrom failed, falling back to full catalog:", msg);
    }
    ...
    for (const bucket of response) {
      ...
      for (const token of bucket.tokens) {
        ...
        if (!token || typeof token.identifier !== "string" || typeof token.chain !== "string") {
          continue;
        }
