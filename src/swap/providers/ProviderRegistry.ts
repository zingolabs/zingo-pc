...
export class ProviderRegistry {
  ...
  constructor(executors: readonly ProviderExecutor[]) {
    ...
    for (const executor of executors) {
      if (this.executors.has(executor.provider)) {
        throw new Error(`ProviderRegistry: duplicate executor for provider "${executor.provider}".`);
      }
  ...
  get(provider: SwapKitProviderEnum): ProviderExecutor {
    ...
    if (!executor) {
      throw new Error(`ProviderRegistry: no executor registered for provider "${provider}".`);
    }
    return executor;
  }
