...
export class NearIntentsExecutor implements ProviderExecutor {
  ...
  extractDepositInstructions(context: ExtractDepositInstructionsContext): DepositInstructionsType {
    ...
    if (!depositAddress) {
      try {
        console.log("NearIntentsExecutor: /v3/swap response shape", JSON.stringify(swapResponse, null, 2));
      } catch {
        console.log("NearIntentsExecutor: /v3/swap response (unstringifiable):", swapResponse);
      }
      throw new Error(
    ...
    const swapKitDepositChannelId =
      ...
      readNestedString(swapResponse.transient?.providerDetails, "depositChannelId") ??
...
function readNestedString(obj: Record<string, unknown> | undefined, key: string): string | undefined {
