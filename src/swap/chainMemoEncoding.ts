...
export const UTXO_SOURCE_CHAINS: ReadonlyArray<string> = ["BTC", "BCH", "LTC", "DOGE", "DASH", "ZEC"];
 ...
 */
export function humanDecimalToBaseUnits(decimalStr: string, decimals: number): string | null {
...
export function buildEip681Uri(args: {
  ...
  return `ethereum:${args.vaultAddress}@${args.chainId}?value=${wei}&data=${args.memoHexWithPrefix}`;
}
...
export function buildMemolessPaymentUri(args: {
  ...
  if (chain === "TON") {
    ...
    const nano = humanDecimalToBaseUnits(args.amountHumanDecimal, args.decimals);
