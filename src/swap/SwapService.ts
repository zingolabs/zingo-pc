...
export class SwapService {
   ...
   */
  async listRoutableTokens(direction: "outbound" | "inbound"): Promise<TokenEntryType[]> {
  ...
  async commitRoute(args: CommitRouteArgs): Promise<CommitRouteResult> {
    ...
    const now = Date.now();
    const initialStatus =
      direction === SwapDirectionEnum.Outbound ? SwapStatusEnum.PendingDeposit : SwapStatusEnum.AwaitingExternalDeposit;
    ...
    const record: SwapRecordType = {
      ...
      broadcast:
        direction === SwapDirectionEnum.Outbound ? { status: BroadcastStatusEnum.PendingBroadcast } : undefined,
      fiatValueBasis,
      createdAtMs: now,
      updatedAtMs: now,
    };
  ...
  async markBroadcasted(args: MarkBroadcastedArgs): Promise<SwapRecordType> {
    ...
    if (record.direction !== SwapDirectionEnum.Outbound) {
      throw new Error(`SwapService.markBroadcasted: not an outbound record (${recordId})`);
    }
    const now = Date.now();
   ...
   */
  async setObservedDepositTxHash(args: SetObservedDepositTxHashArgs): Promise<SwapRecordType> {
    const { recordId, hash } = args;
    const record = await this.store.getByRecordId(recordId);
    if (!record) {
      throw new Error(`SwapService.setObservedDepositTxHash: no record for ${recordId}`);
    }
    if (record.direction !== SwapDirectionEnum.Inbound) {
      throw new Error(`SwapService.setObservedDepositTxHash: not an inbound record (${recordId})`);
    }
    const now = Date.now();
    const updated: SwapRecordType = {
      ...record,
...
function toRouteOption(
  ...
  const receivePerSellRate =
    Number.isFinite(sellAmountNum) && Number.isFinite(buyAmountNum) && sellAmountNum > 0
      ? buyAmountNum / sellAmountNum
      : undefined;

  const convertToReceive = (amount: number, asset: string | undefined): number => {
  ...
  const convertToSell = (amount: number, asset: string | undefined): number => {
    ...
    if (asset === receiveAssetId && receivePerSellRate !== undefined && receivePerSellRate > 0) {
      return amount / receivePerSellRate;
    }
    return 0;
  };

  const totalFeesInReceiveAsset = sumConverted(fees, () => true, convertToReceive);
  ...
  const tags: ReadonlyArray<string> | undefined =
    Array.isArray(rawTags) && rawTags.every((t) => typeof t === "string") ? (rawTags as string[]) : undefined;
  ...
  return {
    ...
    feesRaw: route.fees,
    estimatedTimeText:
      estimatedSecondsTotal !== undefined ? `~${Math.max(1, Math.ceil(estimatedSecondsTotal / 60))} min` : undefined,
    expiresAtMs: Number.isFinite(expirationSecs) ? expirationSecs * 1000 : undefined,
    warningsText: route.warnings && route.warnings.length > 0 ? route.warnings.join("; ") : undefined,
    tags,
  };
}
...
function sumConverted(
  ...
  predicate: (fee: { type?: string; amount?: string; asset?: string }) => boolean,
 ...
 * and paid, so `/track` has a transaction to answer about. An abandoned or
 * expired swap that never saw a deposit has none, and asking would only log
 * a failure.
 */
function needsBackfill(record: SwapRecordType): boolean {
  ...
  return !!(record.broadcast?.txId || record.observedDepositTxHash);
}
