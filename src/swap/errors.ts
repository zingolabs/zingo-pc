...
export class SwapKitHttpError extends SwapKitError {
  ...
  constructor(args: { operation: SwapOperationEnum; httpStatus: number; body: string }) {
 ...
 */
function classifyHttpStatus(operation: SwapOperationEnum, httpStatus: number, body: string): SwapErrorCategoryEnum {
 ...
 */
function classify4xxBody(operation: SwapOperationEnum, body: string): SwapErrorCategoryEnum {
  const lower = body.toLowerCase();
  ...
  if (lower.includes("no quote") || lower.includes("no route") || lower.includes("liquidity")) {
    return SwapErrorCategoryEnum.NoQuoteOrLiquidity;
  }
