...
export class SwapPoller {
   ...
   */
  private async maybeDiscoverHashViaMidgard(record: SwapRecordType, now: number): Promise<SwapRecordType | null> {
    ...
    try {
      ...
      console.log(`SwapPoller: Midgard probe failed for ${record.depositAddress} (failure #${count}): ${errMsg}`);
      return null;
    }
    ...
    const updated: SwapRecordType = {
      ...
      status: record.status === SwapStatusEnum.AwaitingExternalDeposit ? SwapStatusEnum.Pending : record.status,
    ...
    try {
      ...
      const errMsg = err instanceof Error ? err.message : String(err);
      console.log(`SwapPoller: store.upsert (Midgard discovery) failed for ${record.depositAddress}:`, errMsg);
  ...
  private async pollRecord(record: SwapRecordType, callerGeneration: number): Promise<void> {
    ...
    const discoveredRecord = await this.maybeDiscoverHashViaMidgard(record, Date.now());
    if (!this.isRunningForGeneration(callerGeneration)) return;
    ...
    try {
      ...
      const errMsg = err instanceof Error ? err.message : String(err);
      console.log(`SwapPoller: /track failed for ${effectiveRecord.depositAddress} (failure #${count}):`, errMsg, err);
      return;
    }
    ...
    try {
      ...
      const errMsg = err instanceof Error ? err.message : String(err);
      console.log(`SwapPoller: store.upsert failed for ${effectiveRecord.depositAddress}:`, errMsg, err);
    }
  }
}
...
function buildTrackParams(record: SwapRecordType): {
  ...
  const hash = record.broadcast?.txId ?? record.observedDepositTxHash ?? undefined;
 ...
 * paid for.
 */
function hasMeaningfulChange(prev: SwapRecordType, next: SwapRecordType): boolean {
...
function stripNoise(record: SwapRecordType): string {
  return JSON.stringify(record, (key, value) => (NOISE_KEYS.has(key) ? undefined : value));
}
